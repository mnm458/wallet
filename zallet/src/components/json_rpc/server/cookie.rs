...
pub(crate) fn generate_cookie(datadir: &Path) -> Result<(String, PasswordHash), Error> {
    ...
    Ok((COOKIE_USER.to_string(), password_hash))
}
...
pub(crate) fn delete_cookie(datadir: &Path) {
    let path = datadir.join(COOKIE_FILENAME);
    if let Err(e) = fs::remove_file(&path) {
        if e.kind() != std::io::ErrorKind::NotFound {
            warn!("Failed to remove cookie file {}: {}", path.display(), e);
        }
    }
}
...
mod tests {
    ...
    fn generate_creates_file() {
        ...
        generate_cookie(dir.path()).unwrap();
    ...
    fn generate_no_tmp_leftover() {
        let dir = TempDir::new().unwrap();
        let _ = generate_cookie(dir.path()).unwrap();
    ...
    fn delete_removes_file() {
        ...
        generate_cookie(dir.path()).unwrap();
        assert!(dir.path().join(COOKIE_FILENAME).exists());
        delete_cookie(dir.path());
        assert!(!dir.path().join(COOKIE_FILENAME).exists());
    }

    #[test]
    fn delete_missing_file_no_panic() {
        let dir = TempDir::new().unwrap();
        delete_cookie(dir.path());
    }
